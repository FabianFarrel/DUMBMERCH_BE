...
export async function createProduct(data: createProductDto, categoryId: number) {
    const product = await productRepo.createProduct({...data, price: Number(data.price), Qty: Number(data.Qty)}, categoryId);
    if (!product) {
